...
class _StockHomeViewState extends State<StockHomeView> {
  ...
  int _currentStockColumnIndex = 4;
  ...
  Future<void> _loadSavedData() async {
    ...
    if (headersString != null) {
      ...
      _currentStockColumnIndex = prefs.getInt('eb_stock_idx') ?? 4;
    }
  }
      ...
      try {
        ...
        for (var table in excel.tables.keys) {
          ...
          for (int i = 0; i < rows[0].length; i++) {
            ...
            if (headerVal == 'SKU') skuIdx = i;
            if (headerVal.contains('PURCHASE PRICE')) priceIdx = i;
            if (headerVal == 'CATEGORY') catIdx = i;
            if (headerVal.contains('CURRENT STOCK')) stockIdx = i;
          }

          if (skuIdx == -1) {
            // Fallback to absolute indexes if column names are slightly styled or missing
            nameIdx = 0; skuIdx = 1; priceIdx = 2; catIdx = 3; stockIdx = 4;
            tempHeaders = ['Name', 'SKU', 'Purchase price (including tax)', 'Category', 'Current Stock'];
          }
          ...
          for (int i = 1; i < rows.length; i++) {
            if (rows[i].length <= skuIdx) continue;
            
            var rawRow = rows[i].map((e) => e?.value?.toString() ?? '').toList();
            var sku = rows[i][skuIdx]?.value?.toString().trim();
            var name = nameIdx < rows[i].length ? (rows[i][nameIdx]?.value?.toString() ?? 'Unknown') : 'Unknown';
            var priceStr = priceIdx < rows[i].length ? (rows[i][priceIdx]?.value?.toString() ?? '0.0') : '0.0';
            var cat = (catIdx != -1 && catIdx < rows[i].length) ? (rows[i][catIdx]?.value?.toString() ?? 'OTHERS') : 'OTHERS';
  ...
  Future<void> _exportPOSMasterExcel() async {
    ...
    // 1. Setup exact template columns
    List<String> exportHeaders = ['Name', 'SKU', 'Purchase price (including tax)', 'Category', 'Current Stock'];
    for (int col = 0; col < exportHeaders.length; col++) {
      sheet.updateCell(ex.CellIndex.indexByColumnRow(columnIndex: col, rowIndex: 0), ex.CellValue.withValue(exportHeaders[col]));
    }

    Map<String, int> scannedMap = { for (var item in _stockList) item.barcode : item.quantity };
    int rowIndex = 1;
    
    // 2. Loop original master records to keep their exact arrangement and update quantities
    if (_posProductMaster.isNotEmpty) {
      _posProductMaster.forEach((sku, details) {
        String name = details['name'] ?? '';
        double price = details['price'] ?? 0.0;
    ...
    // 3. Include any items scanned but not present in original template master
    ...
    final file = File('${directory.path}/Updated_Master_Format_Stock.xlsx');
    await file.writeAsBytes(fileBytes!);

    setState(() { _isLoading = false; });
    await Share.shareXFiles([XFile(file.path)], text: 'Exported Master Template');
  }
